...
class SAINT(nn.Module):
    def __init__(
        ...
        *,
        ...
        self.simple_MLP = nn.ModuleList(
                [nn.ModuleList(
                    [simple_MLP([1,100,self.dim]) for _ in range(num_continuous)]
                )]
            )
        ...
        self.specific_classifier = nn.ModuleList([simple_MLP([2 * dim ,1000, y_dim])])
    ...
    def add_task(self, categories, num_continuous, y_dim):
        ...
        self.simple_MLP.append(
            nn.ModuleList([simple_MLP([1,100,self.dim]) for _ in range(num_continuous)])
        )
        ...
        self.specific_classifier.append(simple_MLP([2 * self.dim ,1000, y_dim]))
