...
class SAINT(nn.Module):
    def __init__(
        ...
        *,
        ...
        if condition == None:
            self.specific_classifier = nn.ModuleList([simple_MLP([2 * dim, int(self.hidden_dims), y_dim])])
        else:
            self.specific_classifier = nn.ModuleList([simple_MLP([dim, int(self.hidden_dims), y_dim])])      
    ...
    def add_task(self, categories, num_continuous, y_dim):
        ...
        if self.condition == None:
            self.specific_classifier.append(simple_MLP([2 * self.dim, int(self.hidden_dims), y_dim]))
        else:
            self.specific_classifier.append(simple_MLP([self.dim, int(self.hidden_dims), y_dim]))
